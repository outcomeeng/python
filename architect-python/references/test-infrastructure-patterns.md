...
Run the product's canonical test command — the one its active root harness guide, Justfile, Makefile, or package scripts document. The direct `python3 -m pytest` invocations below are the portable fallback for a product that ships no wrapper.
